-- ============================================
-- MI HISTORIA CLINICA - INICIALIZACIÓN COMPLETA
-- ============================================
...
-- ==================== TURNOS ====================
DROP PROCEDURE IF EXISTS generar_turnos_medico;
DELIMITER //

CREATE PROCEDURE generar_turnos_medico(
    IN p_medic_id BIGINT,
    IN p_medic_name VARCHAR(255),
    IN p_specialty INT,
    IN p_center INT,
    IN p_dias VARCHAR(20),
    IN p_hora_inicio INT,
    IN p_hora_fin INT,
    IN p_duracion INT
)
BEGIN
    ...
    SET v_fecha = CURDATE();
    SET v_fecha_fin = DATE_ADD(CURDATE(), INTERVAL 3 MONTH);
    
    WHILE v_fecha <= v_fecha_fin DO
        SET v_dia_semana = DAYOFWEEK(v_fecha);
        
        IF (v_dia_semana = 2 AND INSTR(p_dias, 'L') > 0) OR
           (v_dia_semana = 3 AND INSTR(p_dias, 'M') > 0) OR
           (v_dia_semana = 4 AND INSTR(p_dias, 'X') > 0) OR
           (v_dia_semana = 5 AND INSTR(p_dias, 'J') > 0) OR
           (v_dia_semana = 6 AND INSTR(p_dias, 'V') > 0) THEN
            
            SET v_hora = p_hora_inicio;
            SET v_minuto = 0;
            
            WHILE (v_hora * 60 + v_minuto) < (p_hora_fin * 60) DO
                INSERT INTO turnos (fecha_turno, hora_turno, medic_id, medic_full_name, medic_specialty, medical_center, available)
                VALUES (v_fecha, MAKETIME(v_hora, v_minuto, 0), p_medic_id, p_medic_name, p_specialty, p_center, TRUE);
                
                SET v_minuto = v_minuto + p_duracion;
                IF v_minuto >= 60 THEN
                    SET v_hora = v_hora + FLOOR(v_minuto / 60);
                    SET v_minuto = v_minuto MOD 60;
                END IF;
            END WHILE;
        END IF;
...
-- Generar turnos para todos los médicos
CALL generar_turnos_medico(1, 'Carlos González', 0, 0, 'LXV', 8, 13, 30);
CALL generar_turnos_medico(2, 'Ana Martínez', 0, 1, 'MJ', 14, 19, 30);
CALL generar_turnos_medico(3, 'Roberto López', 0, 2, 'LMXJV', 9, 14, 30);
CALL generar_turnos_medico(4, 'Laura Sánchez', 1, 0, 'LXV', 9, 13, 20);
CALL generar_turnos_medico(5, 'Jorge Pérez', 1, 1, 'MJ', 14, 18, 20);
CALL generar_turnos_medico(6, 'Sofía García', 1, 3, 'LMX', 8, 12, 20);
CALL generar_turnos_medico(7, 'Juliana Herrera', 10, 0, 'LMXJV', 8, 12, 20);
CALL generar_turnos_medico(8, 'Matías Medina', 10, 0, 'LMXJV', 14, 18, 20);
CALL generar_turnos_medico(9, 'Victoria Núñez', 10, 1, 'LXV', 9, 13, 20);
CALL generar_turnos_medico(10, 'Lucas Rojas', 10, 2, 'MJ', 14, 19, 20);
CALL generar_turnos_medico(11, 'Florencia Campos', 10, 3, 'LMX', 8, 13, 20);
CALL generar_turnos_medico(12, 'Sandra Miranda', 20, 0, 'LMXJV', 8, 13, 20);
CALL generar_turnos_medico(13, 'Claudio Figueroa', 20, 0, 'LMXJV', 14, 19, 20);
CALL generar_turnos_medico(14, 'Mariana Ledesma', 20, 1, 'LXV', 9, 14, 20);
CALL generar_turnos_medico(15, 'Leonardo Peralta', 20, 2, 'MJ', 8, 13, 20);
CALL generar_turnos_medico(16, 'Alejandra Ibarra', 20, 3, 'LMX', 14, 18, 20);
CALL generar_turnos_medico(17, 'Alejandro Vega', 14, 0, 'LXV', 8, 14, 30);
CALL generar_turnos_medico(18, 'Jimena Luna', 14, 1, 'MJ', 9, 14, 30);
CALL generar_turnos_medico(19, 'Rodrigo Ríos', 14, 2, 'LMXJV', 14, 18, 30);
CALL generar_turnos_medico(20, 'Micaela Benítez', 14, 3, 'LMX', 8, 13, 30);
CALL generar_turnos_medico(21, 'Patricia Aguilar', 16, 0, 'LXV', 8, 13, 30);
CALL generar_turnos_medico(22, 'Adriana Molina', 16, 1, 'MJ', 14, 18, 30);
CALL generar_turnos_medico(23, 'Verónica Cabrera', 16, 2, 'LMXJV', 9, 14, 30);
CALL generar_turnos_medico(24, 'Daniela Reyes', 16, 3, 'LMX', 14, 19, 30);
CALL generar_turnos_medico(25, 'Sebastián Ramírez', 6, 0, 'LX', 8, 14, 40);
CALL generar_turnos_medico(26, 'Carolina Mendoza', 6, 1, 'MJV', 9, 14, 40);
CALL generar_turnos_medico(27, 'Nicolás Gutiérrez', 6, 2, 'LXV', 14, 18, 40);
CALL generar_turnos_medico(28, 'Gabriela Vargas', 6, 3, 'MJ', 8, 13, 40);
CALL generar_turnos_medico(29, 'Federico Silva', 3, 0, 'LXV', 8, 13, 30);
CALL generar_turnos_medico(30, 'Luciana Morales', 3, 1, 'MJ', 14, 18, 30);
CALL generar_turnos_medico(31, 'Emiliano Castro', 3, 2, 'LMXJV', 9, 14, 30);
CALL generar_turnos_medico(32, 'Mariela Acosta', 11, 0, 'LXV', 9, 14, 45);
CALL generar_turnos_medico(33, 'Gustavo Paz', 11, 1, 'MJ', 14, 19, 45);
CALL generar_turnos_medico(34, 'Cecilia Sosa', 11, 2, 'LMXJV', 10, 15, 45);
CALL generar_turnos_medico(35, 'Facundo Ramos', 8, 0, 'LXV', 8, 13, 20);
CALL generar_turnos_medico(36, 'Antonella Ponce', 8, 1, 'MJ', 14, 18, 20);
CALL generar_turnos_medico(37, 'Maximiliano Navarro', 8, 2, 'LMXJV', 9, 14, 20);
CALL generar_turnos_medico(38, 'Valeria Torres', 2, 0, 'LXV', 8, 13, 30);
CALL generar_turnos_medico(39, 'Martín Flores', 2, 1, 'MJ', 14, 18, 30);
CALL generar_turnos_medico(40, 'Camila Díaz', 2, 3, 'LMX', 9, 14, 30);
CALL generar_turnos_medico(41, 'Daniel Escobar', 15, 0, 'LXV', 8, 13, 25);
CALL generar_turnos_medico(42, 'Paola Giménez', 15, 1, 'MJ', 14, 18, 25);
CALL generar_turnos_medico(43, 'Ricardo Quiroga', 18, 0, 'LX', 9, 13, 30);
CALL generar_turnos_medico(44, 'Mónica Zárate', 18, 1, 'MV', 10, 14, 30);
CALL generar_turnos_medico(45, 'Guillermo Coronel', 18, 2, 'JV', 8, 12, 30);
CALL generar_turnos_medico(46, 'Teresa Delgado', 9, 0, 'LXV', 8, 13, 25);
CALL generar_turnos_medico(47, 'Eduardo Soto', 9, 1, 'MJ', 14, 18, 25);
CALL generar_turnos_medico(48, 'Oscar Montenegro', 13, 0, 'LXV', 9, 14, 30);
CALL generar_turnos_medico(49, 'Elena Cortés', 13, 2, 'MJ', 14, 18, 30);
CALL generar_turnos_medico(50, 'Cristian Bustos', 7, 0, 'LMX', 8, 14, 45);

DROP PROCEDURE IF EXISTS generar_turnos_medico;

-- ==================== RESUMEN FINAL ====================
SELECT '========== RESUMEN DE DATOS CARGADOS ==========' AS info;
SELECT 'Médicos' AS tabla, COUNT(*) AS cantidad FROM medic
UNION ALL SELECT 'Pacientes', COUNT(*) FROM patient
UNION ALL SELECT 'Relaciones médico-paciente', COUNT(*) FROM patient_medic
UNION ALL SELECT 'Historias clínicas', COUNT(*) FROM medical_file
UNION ALL SELECT 'Consultas médicas', COUNT(*) FROM medical_appointment
UNION ALL SELECT 'Medicamentos', COUNT(*) FROM medicine
UNION ALL SELECT 'Análisis', COUNT(*) FROM analysis
UNION ALL SELECT 'Turnos disponibles', COUNT(*) FROM turnos;

SELECT '========== TURNOS POR ESPECIALIDAD ==========' AS info;
SELECT 
    CASE medic_specialty 
        WHEN 0 THEN 'Cardiología'
        WHEN 1 THEN 'Dermatología'
        WHEN 2 THEN 'Endocrinología'
        WHEN 3 THEN 'Gastroenterología'
        WHEN 6 THEN 'Neurología'
        WHEN 7 THEN 'Oncología'
        WHEN 8 THEN 'Oftalmología'
        WHEN 9 THEN 'Otorrinolaringología'
        WHEN 10 THEN 'Pediatría'
        WHEN 11 THEN 'Psiquiatría'
        WHEN 13 THEN 'Reumatología'
        WHEN 14 THEN 'Traumatología'
        WHEN 15 THEN 'Urología'
        WHEN 16 THEN 'Ginecología'
        WHEN 18 THEN 'Cirugía General'
        WHEN 20 THEN 'Medicina Clínica'
    END AS especialidad,
    COUNT(*) AS turnos,
    COUNT(DISTINCT medic_id) AS medicos
FROM turnos 
GROUP BY medic_specialty 
